...
毎日続けたい習慣を登録し、今日の実行をチェックで記録する静的単一ページ Web アプリ。状態は localStorage（キー `habit-tracker:v1`）に `{ habits, checks }` として保存され、チェックはローカル日付 `YYYY-MM-DD` ごとに保持する。各習慣行には直近7日の履歴ドットと連続実行日数バッジを表示する。
